...
adat$week <- week(adat$acquisition_time)
table(adat$week)
adat$acquisition_time <- as.POSIXct(adat$acquisition_time)

# Interpolate at 5 min intervals
ex <- as.data.frame( #extremes of time for each individual in each week
  adat %>% 
  group_by(animals_id, year, week) %>%
  summarise(min_t = min(acquisition_time), max_t = max(acquisition_time)) %>% 
  ungroup() %>% 
  mutate(group = paste(animals_id, year, week))  
  )
...
adat$group <- paste(adat$animals_id, adat$year, adat$week)
...
#can ignore warning for repeated x
...
write.csv(adat_m, "data/Activity_data_byWeek_5min.csv", row.names=F)
